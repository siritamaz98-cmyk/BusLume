...
// BusLume ESP32 USB-CAN adapter v15 MONITOR + SPEED
// ESP32-WROOM-32 <-> TXS0108E <-> MCP2515 (8 MHz crystal)
...
// All rates provided by the MCP2515 library for an 8 MHz crystal.  The
// SLCAN code is kept where the standard SLCAN table has one; the other rates
// are available from the local MONITOR menu.
...
const CanSpeedOption CAN_SPEEDS[] = {
  ...
  {"10K",  CAN_10KBPS,    0},
  {"20K",  CAN_20KBPS,    1},
  {"31.25K", CAN_31K25BPS, 0xFF},
  {"33.33K", CAN_33KBPS,   0xFF},
  {"40K",  CAN_40KBPS,    0xFF},
  {"50K",  CAN_50KBPS,    2},
  {"80K",  CAN_80KBPS,    0xFF},
  {"100K", CAN_100KBPS,   3},
  {"125K", CAN_125KBPS,   4},
  {"200K", CAN_200KBPS,   0xFF},
  {"250K", CAN_250KBPS,   5},
  {"500K", CAN_500KBPS,   6},
  {"1M",   CAN_1000KBPS,  8}
};
...
bool applyCanSpeed(byte index) {
  ...
  // setBitrate() puts the MCP2515 into configuration mode.  Restore the
  // current mode immediately when the controller is already open; during
  // boot/recovery MODE_CLOSED leaves it in configuration mode for filters.
  if (mcp2515.setBitrate(CAN_SPEEDS[index].value, MCP_8MHZ)
      != MCP2515::ERROR_OK) {
    return false;
  }
  if (adapterMode != MODE_CLOSED && !restoreMode()) return false;
...
bool restoreMode() {
  ...
  if (adapterMode == MODE_NORMAL) result = mcp2515.setNormalMode();
  else if (adapterMode == MODE_LISTEN_ONLY) result = mcp2515.setListenOnlyMode();
  else if (adapterMode == MODE_LOOPBACK) result = mcp2515.setLoopbackMode();
  else result = mcp2515.setConfigMode();
  return result == MCP2515::ERROR_OK;
}

bool applyAcceptanceFilter() {
  if (mcp2515.setFilterMask(MCP2515::MASK0, false, filterMask) != MCP2515::ERROR_OK) {
    return false;
  }

  const MCP2515::RXF filters[6] = {
    MCP2515::RXF0, MCP2515::RXF1, MCP2515::RXF2,
    MCP2515::RXF3, MCP2515::RXF4, MCP2515::RXF5
  };

  for (byte i = 0; i < 6; i++) {
    if (mcp2515.setFilter(filters[i], false, filterCode) != MCP2515::ERROR_OK) {
      return false;
    }
  }
...
bool parseAndSendFrame(const char *command) {
  ...
  lastErrorFlags = mcp2515.getErrorFlags();
  errorCount++;
...
void processCommand(const char *command) {
  ...
  if (command[0] == 'S' && command[1] != '\0' && command[2] == '\0') {
    AdapterMode previousMode = adapterMode;
    if (applyBitrate(command[1] - '0') &&
        (previousMode == MODE_CLOSED || restoreMode())) {
      commandReply->write('\r');
  ...
  if (command[0] == 'O' || command[0] == 'L' || command[0] == 'Q') {
    ...
    sendAllActive = false;
    MCP2515::ERROR result = MCP2515::ERROR_FAIL;
    if (command[0] == 'O') result = mcp2515.setNormalMode();
    if (command[0] == 'L') result = mcp2515.setListenOnlyMode();
    if (command[0] == 'Q') result = mcp2515.setLoopbackMode();

    if (result == MCP2515::ERROR_OK) {
      adapterMode = command[0] == 'O' ? MODE_NORMAL :
                    command[0] == 'L' ? MODE_LISTEN_ONLY : MODE_LOOPBACK;
      commandReply->write('\r');
  ...
  // F returns the MCP2515 error flags as four hexadecimal digits.
...
void checkMcpErrors() {
  if (adapterMode == MODE_CLOSED) return;
  byte flags = mcp2515.getErrorFlags();
  if (flags != 0) {
    if (flags != lastErrorFlags) errorCount++;
    lastErrorFlags = flags;
    // Clear receive-overflow flags so reception can continue.
    if (flags & 0xC0) mcp2515.clearRXnOVR();
    bool recovered = (flags & 0x20) && adapterMode == MODE_NORMAL
                   ? recoverCanController()
                   : false;
    setStatus(recovered ? "BUS RECOVERED" : "CAN ERROR");
  }
}

bool recoverCanController() {
  if (adapterMode == MODE_CLOSED) return false;
  if (millis() - lastRecovery < 1000) return false;
  lastRecovery = millis();

  setStatus("BUS RECOVERY");
  if (mcp2515.setConfigMode() != MCP2515::ERROR_OK) return false;
  if (speedIndex >= CAN_SPEED_COUNT) return false;
  // Keep the controller in configuration mode until the filters are
  // restored; applyAcceptanceFilter() returns it to the active mode.
  if (mcp2515.setBitrate(CAN_SPEEDS[speedIndex].value, MCP_8MHZ)
      != MCP2515::ERROR_OK) return false;
  if (!applyAcceptanceFilter()) return false;
  lastErrorFlags = 0;
  return true;
}
...
void readCanFrames() {
  ...
  for (byte i = 0; i < 32; i++) {
    ...
    if (result != MCP2515::ERROR_OK) break;
    rxCount++;
...
void setup() {
  ...
  mcp2515.reset();
  delay(50);
  applyCanSpeed(speedIndex);
  adapterMode = MODE_CLOSED;
  mcp2515.setConfigMode();
  applyAcceptanceFilter();
  loadFramesFromPreferences();

  // Start locally in normal CAN mode. BusLume can still send S6/O over USB.
  adapterMode = MODE_NORMAL;
