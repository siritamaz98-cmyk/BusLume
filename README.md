...
Основні можливості BusLume:

- моніторинг CAN-трафіку в реальному часі;
- передавання CAN-кадрів з налаштуванням ID, DLC, DATA та періоду;
- запис, відтворення, фільтрація та порівняння CAN-трас;
- статистика ID, байтів і бітів, графіки та Bit View;
- GPS-пошук ID швидкості автомобіля;
- автоматичний пошук кандидатів сигналів і контрольоване тестування;
- інструменти для роботи з DBC;
- локальна та спільна бібліотека CAN-сигналів;
- збереження фрагментів трас як доказу для перевірених сигналів;
- підключення через Bluetooth та USB;
- український та англійський інтерфейс.

BusLume активно розробляється та перевіряється на реальних автомобілях. Відгуки, тестування, знайдені CAN-сигнали та приклади трас допомагають розвитку проєкту.

[⬇️ Завантажити останню версію BusLume](https://github.com/siritamaz98-cmyk/BusLume/releases/latest)
